# プロジェクト概要
このプロジェクトはCLI版の画像ファイル格納先フリア訳プリケーションを開発するプロジェクトです。
...
### ドキュメントコメントの指針
ファイル先頭にはコピーライトヘッダを置き、それに続く形で以下の様なモジュール内容を要約したモジュールヘッダを記述してください。
...
プロジェクトの状態を変更する可能性のあるコマンドうち以下のものは使用しても構いません。
